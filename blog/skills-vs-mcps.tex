...
{\color{accent}\small\bfseries DAYTONA}\\[4pt]
...
An MCP server hands an agent a box of tools. A Skill teaches it the craft to use them. They are not rivals --- they are different layers. Today we are adding one to Daytona: the \textbf{Daytona Skill} for Claude.
...
The difference: tools say what an agent \emph{can} do; a skill says \emph{how} to do it well. You want both.

\cake

\textbf{\color{ink}The Daytona Skill.}

It packages everything we have learned about driving secure, sub-90ms sandboxes into a single playbook Claude reads on demand --- about 100 tokens of context until the moment it is needed, instead of a dozen tool definitions sitting in every conversation. And it drives the full Daytona SDK: persistent sessions, snapshots, volumes, and code execution --- the superset of what the MCP exposes.

MCP gives your agent reach. Skills give it judgment. Together: an agent that runs the code it writes --- in a sandbox, and cleans up when it is done.

\vspace{4pt}
{\color{accent}\rule{\linewidth}{0.8pt}}
\vspace{2pt}

\textbf{The Daytona Skill is live.} \quad \href{https://www.daytona.io/docs}{Documentation} \ \textperiodcentered\ \ \href{https://github.com/jiviny/daytona-skill}{GitHub} \ \textperiodcentered\ \ \href{https://app.daytona.io}{Sign up}
